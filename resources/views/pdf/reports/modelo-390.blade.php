...
@section('title', __('reports.modelo_390_full'))
...
@section('content')
    ...
    <div class="section-header">{{ __('reports.annual_vat_charged') }}</div>
    <table class="report-table">
        <thead>
            <tr>
                <th>{{ __('reports.vat_type') }}</th>
                <th class="text-right">{{ __('reports.col_vat_base') }}</th>
                <th class="text-right">{{ __('reports.col_vat_amount') }}</th>
            </tr>
        </thead>
        <tbody>
    ...
    <div class="section-header">{{ __('reports.annual_vat_deductible') }}</div>
    <table class="report-table">
        <thead>
            <tr>
                <th>{{ __('reports.vat_type') }}</th>
    ...
    {{-- Quarterly breakdown --}}
    <div class="section-header">{{ __('reports.quarterly_breakdown') }}</div>
    <table class="report-table">
        <thead>
            <tr>
                <th>{{ __('reports.col_quarter') }}</th>
                <th class="text-right">{{ __('reports.col_vat_charged') }}</th>
                <th class="text-right">{{ __('reports.col_vat_deductible_short') }}</th>
                <th class="text-right">{{ __('reports.col_difference') }}</th>
            </tr>
        </thead>
        <tbody>
            @foreach($quarters as $q)
                <tr>
                    <td>{{ $q['quarter'] }}T</td>
                    <td class="text-right">{{ number_format($q['vat_issued'], 2, ',', '.') }} &euro;</td>
                    <td class="text-right">{{ number_format($q['vat_received'], 2, ',', '.') }} &euro;</td>
                    <td class="text-right" style="font-weight: bold; color: {{ $q['difference'] >= 0 ? '#b91c1c' : '#15803d' }};">
                        {{ number_format($q['difference'], 2, ',', '.') }} &euro;
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <div class="summary-box">
        <table style="width: 100%;">
            ...
            <tr>
                <td style="border-top: 2px solid #2563eb; padding-top: 6px; font-size: 10pt; font-weight: bold;">{{ __('reports.annual_result') }}</td>
