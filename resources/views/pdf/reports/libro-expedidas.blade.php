        ...
        <tbody>
            @forelse($data as $row)
                ...
                @if(!empty($row['vat_breakdown']))
                    <tr>
                        <td colspan="10" style="padding-left: 30px; font-size: 7pt; color: #6b7280; background-color: #f9fafb;">
            ...
            @empty
                <tr>
                    <td colspan="10" style="text-align: center; color: #9ca3af;">{{ __('books.no_data') }}</td>
                </tr>
            @endforelse
        </tbody>
        @if(count($data) > 0)
