...
@section('content')
    <table class="report-table">
        <thead>
            <tr>
                <th>{{ __('books.col_reception_number') }}</th>
        ...
        <tbody>
            @forelse($data as $row)
                <tr>
                    ...
                    <td class="text-right">{{ number_format((float) ($row['total_surcharge'] ?? 0), 2, ',', '.') }} &euro;</td>
                    <td class="text-right">{{ (float) ($row['irpf_amount'] ?? 0) != 0 ? number_format((float) $row['irpf_amount'], 2, ',', '.') . ' €' : '' }}</td>
