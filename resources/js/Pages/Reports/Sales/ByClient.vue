...
const pdfUrl = () => route('reports.sales.by-client.pdf', {
    date_from: dateFrom.value,
    date_to: dateTo.value,
});
...
<template>
    <Head :title="$t('reports.sales_by_client')" />

    <AppLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-900">{{ $t('reports.sales_by_client') }}</h1>
        </template>
        ...
        <div class="overflow-hidden rounded-lg bg-white shadow">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ $t('reports.col_client') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ $t('reports.col_nif') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="(row, i) in data" :key="row.client_id ?? i" class="hover:bg-gray-50">
                        <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900">
                            {{ row.client?.trade_name || row.client?.legal_name || $t('common.no_client') }}
                        </td>
                    ...
                    <tr v-if="data.length === 0">
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-400">{{ $t('reports.no_data') }}</td>
                    </tr>
                </tbody>
