...
const applyFilters = () => {
    router.get(route('reports.fiscal.modelo-111'), {
        year: year.value,
...
const pdfUrl = () => route('reports.fiscal.modelo-111.pdf', { year: year.value, quarter: quarter.value });
const csvUrl = () => route('reports.fiscal.modelo-111.csv', { year: year.value, quarter: quarter.value });
</script>

<template>
    <Head :title="$t('reports.modelo_111')" />

    <AppLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-900">{{ $t('reports.modelo_111_full') }}</h1>
        </template>
        ...
        <!-- Section: Professional withholdings -->
        <div class="rounded-lg bg-white p-6 shadow">
            <h3 class="mb-4 text-sm font-semibold text-gray-900">{{ $t('reports.section_professional') }}</h3>

            <div v-if="data.recipients > 0" class="space-y-4">
                <div class="flex justify-between border-b border-gray-100 pb-3">
                    <span class="text-sm text-gray-600">{{ $t('reports.casilla_04_recipients') }}</span>
                    <span class="text-sm font-semibold text-gray-900">{{ data.recipients }}</span>
                </div>
                <div class="flex justify-between border-b border-gray-100 pb-3">
                    <span class="text-sm text-gray-600">{{ $t('reports.casilla_05_base') }}</span>
                    <span class="text-sm font-semibold text-gray-900">{{ formatCurrency(data.base) }}</span>
                </div>
                <div class="flex justify-between border-b border-gray-100 pb-3">
                    <span class="text-sm text-gray-600">{{ $t('reports.casilla_06_withheld') }}</span>
