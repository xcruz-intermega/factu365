            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-2 py-2 text-left text-xs font-medium uppercase text-gray-500">{{ $t('books.col_supplier') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <template v-for="group in monthGroups()" :key="group.key">
                        ...
                        <tr class="bg-blue-50">
                            <td colspan="4" class="px-2 py-1.5 text-xs font-medium text-blue-800">{{ $t('books.monthly_subtotal', { month: group.label }) }}</td>
                            <td class="px-2 py-1.5 text-right text-xs font-medium text-blue-800">{{ formatCurrency(group.totals.tax_base) }}</td>
                            <td class="px-2 py-1.5 text-right text-xs font-medium text-blue-800">{{ formatCurrency(group.totals.total_vat) }}</td>
                ...
                <tfoot v-if="data.length > 0" class="bg-gray-50 font-semibold">
                    <tr>
                        <td class="px-2 py-2 text-xs text-gray-900" colspan="4">{{ $t('books.total_period') }}</td>
