        ...
        <div class="overflow-x-auto rounded-lg bg-white shadow">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-2 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ $t('books.col_reception_number') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <template v-for="(row, idx) in data" :key="idx">
                        <tr class="cursor-pointer hover:bg-gray-50" @click="toggleExpand(idx)">
                            <td class="whitespace-nowrap px-2 py-2 text-sm text-gray-500">{{ row.reception_number }}</td>
                            ...
                            <td class="whitespace-nowrap px-2 py-2 text-right text-sm text-gray-500">{{ row.irpf_amount ? formatCurrency(row.irpf_amount) : '' }}</td>
                    ...
                    <tr v-if="data.length === 0">
                        <td colspan="12" class="px-2 py-8 text-center text-sm text-gray-400">{{ $t('books.no_data') }}</td>
                    </tr>
                </tbody>
                <tfoot v-if="data.length > 0" class="bg-gray-50 font-semibold">
                    <tr>
                        <td class="px-2 py-3 text-sm text-gray-900" colspan="6">{{ $t('books.total_period') }}</td>
