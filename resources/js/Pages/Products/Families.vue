...
const submitNew = () => {
    newForm.post(route('product-families.store'), {
        onSuccess: () => {
            showNew.value = false;
            newForm.reset();
        },
...
const submitEdit = (id: number) => {
    editForm.put(route('product-families.update', id), {
...
const executeDelete = () => {
    ...
    deleting.value = true;
    router.delete(route('product-families.destroy', deleteTarget.value.id), {
        onFinish: () => {
                        ...
                        <template v-else>
                            ...
                            <td class="px-4 py-2 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button @click="startEdit(family)" class="text-indigo-600 hover:text-indigo-900" :title="$t('common.edit')">
                                    ...
                                    <button @click="confirmDelete(family)" class="text-red-600 hover:text-red-900" :title="$t('common.delete')">
